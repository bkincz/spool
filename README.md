...
`spool upgrade` never overwrites your work. Spool records a hash of every file it writes in `.spool/generated.json`, so a later run can tell its own output from your edits even after the generated output itself changes. A file you have edited is offered, not replaced, and keeping it is remembered so the next run stays quiet. Workspaces created before that record existed get asked once per generated file, since a `Generated by spool` banner sits at the top of a file you may have edited below it and proves nothing on its own. `--force` skips the asking, and without a terminal nothing is ever overwritten.
...
- **ESLint**: one flat config at the workspace root, with `typescript-eslint` plus the plugin each framework you use needs. `pnpm run lint` checks every app and package at once.
- **Vitest**: a `vitest.config.ts` per app, kept separate from `vite.config.ts` because module federation cannot run under a test. Each host resolves its remotes to stubs in `src/test`, regenerated whenever its remote list changes, so a host renders in tests without any remote being served. Every app also gets a `type-check` script, which you get with or without this addon.
